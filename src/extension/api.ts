...
export class API extends EventEmitter<RooCodeEvents> implements RooCodeAPI {
	...
	public readonly sidebarProvider: ClineProvider
	...
	constructor(
		...
		if (socketPath) {
			...
			ipc.on(IpcMessageType.TaskCommand, async (clientId, { commandName, data }) => {
				this.log(`[API] Received command: ${commandName}`);
				switch (commandName) {
					case TaskCommandName.StartNewTask:
						this.log(`[API] StartNewTask -> ${data.text}, ${JSON.stringify(data.configuration)}`)
						await this.handleHeadlessTask(clientId, data)
						break
					...
					case TaskCommandName.CloseTask:
						...
						break
					default:
						this.log(`[API] Unknown command: ${commandName}`);
				}
			})
		}
	}
	...
	public override emit<K extends keyof RooCodeEvents>(
		...
		if (typeof firstArg === "string") {
			taskId = firstArg
		} else if (typeof firstArg === "object" && firstArg !== null && "taskId" in firstArg) {
			taskId = (firstArg as { taskId: string }).taskId
		}

		const task = taskId ? this.sidebarProvider.getTask(taskId) : undefined
		this.log(`[API] emit: eventName=${eventName}, taskId=${taskId}, taskFound=${!!task}, clientId=${task?.clientId}`)
		const data = { eventName: eventName as RooCodeEventName, payload: args } as TaskEvent

		if (task?.clientId && this.ipc) {
			this.log(`[API] emit: Sending to client ${task.clientId}`)
			this.ipc.send(task.clientId, { type: IpcMessageType.TaskEvent, origin: IpcOrigin.Server, data })
		} else {
			this.log(`[API] emit: Broadcasting to all clients`)
	...
	public async startNewTask({
		...
		if (ipc) {
			// This path is not used by the headless client, but we keep it for potential future use.
			// The clientId is not available here, so we can't send targeted messages.
			return this.handleHeadlessTask(undefined, { configuration, text, images });
		}
	...
	public async handleHeadlessTask(
		...
		const taskId = await this.taskRunner.createTask(text, images, clientId)
		this.log(`[API] handleHeadlessTask: Task created with ID: ${taskId}`);
		return taskId
	}
